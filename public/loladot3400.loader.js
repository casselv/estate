...
(function () {
  ...
  function updateClickedObjectsDisplay() {
    ...
    clickedObjectNames.forEach((name) => {
      ...
      removeButton.className = "removeButton"; // Set the button text to 'X'
  ...
  function removeClickedObjectName(objectName) {
    ...
    if (index !== -1) {
      ...
      if (window.unityInstance) {
        ...
        console.error("Unity instance is not ready or failed to load.");
      }
      // Debugging line
    }
  }
  // Expose the setClickedObjectName to the global scope
  window.setClickedObjectName = setClickedObjectName;
})();
